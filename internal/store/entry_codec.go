...
// is parsed but not yet returned; compaction (phase 4) will consume it.
func decodeTableEntry(val []byte) (e Entry, evicted bool, err error) {
	if len(val) < 1 {
		return Entry{}, false, fmt.Errorf("%w: empty value", errBadTableEntry)
	}
	switch val[0] {
	case tableKindEvict:
	...
	case tableKindEntry:
	default:
		return Entry{}, false, fmt.Errorf("%w: unknown kind 0x%02x", errBadTableEntry, val[0])
	}
	r := bytes.NewReader(val[1:])
	sibCount, err := readUint16(r)
	if err != nil {
		return Entry{}, false, fmt.Errorf("%w: sib count: %v", errBadTableEntry, err)
	}
	sibs := make([]Sibling, sibCount)
	for i := range sibs {
		sib, err := readSibling(r, i)
		if err != nil {
			return Entry{}, false, fmt.Errorf("%w: %v", errBadTableEntry, err)
		}
		sibs[i] = sib
	}
	if _, _, err := readTombstoneAge(r); err != nil {
		return Entry{}, false, fmt.Errorf("%w: %v", errBadTableEntry, err)
	}
	return Entry{Siblings: sibs}, false, nil
}
