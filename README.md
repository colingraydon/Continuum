...
Clients that need more than eventual consistency opt in per request. Conditional writes run a single-decree Paxos round per key, Cassandra-LWT style with promises fsynced to their own log, and reject on a clock mismatch instead of forking a sibling. Serial reads ride that same prepare phase for linearizability. A session-clock header buys read-your-writes and monotonic reads.

A porcupine-based checker holds the CAS path to that claim: racing-client histories must linearize per key, through kills, restarts, and asymmetric partitions, in both the process-level fault harness and a seeded in-process simulation.
...
- **TLA+ specification** - shipped: `specs/Replication.tla` models quorum writes, crash/recovery, hinted handoff, anti-entropy, tombstones and the downtime gate, and TLC checks `NoResurrection` and `Durability` exhaustively over the bounded state space (`make spec`). It proved both tombstone-GC guards load-bearing and showed that a downtime-gate wipe costs durability budget exactly as a permanent failure does - a stronger statement than the prose made. Writes fan out **per replica**, so the model admits partial delivery and a replica crashing mid-fan-out. **Trace conformance** (`make spec-trace`) closes the loop: a recorded execution of a real cluster is replayed against the model, so a spec that drifts from the implementation is caught rather than quietly becoming fiction. It caught an unrealistic event ordering on its first run, and rejects corrupted traces. See [docs/tla-spec.md](docs/tla-spec.md)
...
- **Multi-DC replication** - shipped: per-DC replica counts (`REPLICATION_FACTOR_BY_DC`) drive placement, so a key keeps a full replica set in each DC, and `?consistency=local_quorum` acknowledges from the coordinator's own DC alone - a severed WAN link no longer fails those writes. Both the simulation and fault harnesses run cross-DC partition scenarios proving it. The cross-DC delivery path is hardened too: a WAN timeout budget for remote replica hops, anti-entropy paced separately across the WAN (`CROSS_DC_SYNC_EVERY`), and hints dropped undelivered escalating to a targeted repair pass rather than waiting on the background cycle. Still open: `EACH_QUORUM`, and a gossiped runtime topology map in place of static per-DC config; see [docs/multi-dc.md](docs/multi-dc.md)
...
- **Transport security** - node-to-node and client traffic is plaintext HTTP on an assumed-trusted network, and nothing authenticates a node joining the ring or a peer asserting membership over gossip; TLS (or mTLS with per-node certificates) plus cluster-join authorization is the first step. The full posture, including what else is out of scope today, is in [docs/security.md](docs/security.md)
